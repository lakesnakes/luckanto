...
/**
  * @title  luckanto PrizePool
  * @author luckanto Inc Team
...
abstract contract PrizePool is IPrizePool, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 public poolTotalAward;
    uint256 public poolTotalDeposit;
    ...
    function _depositTo(address _operator, address _to, uint256 _amount) internal
    {
        ...
        _token().transferFrom(_operator, address(this), _amount);

        _mint(_to, _amount, _ticket);
        _supply(_amount);

        poolTotalDeposit += _amount;
    ...
    function award(address _to, uint256 _amount) external override onlyPrizeStrategy {
        ...
        uint256 _redeemed = _redeem(_amount);
        _token().transfer(_to, _redeemed);
        poolTotalAward += _amount;
        _mint(_to, _amount, _ticket);
    ...
    function awardExternalERC721(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            try IERC721(_externalToken).transferFrom(address(this), _to, _tokenIds[i]) {
    ...
    function _transferOut(
        ...
        IERC20(_externalToken).transfer(_to, _amount);
