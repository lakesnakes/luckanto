...
contract DrawBeacon is IDrawBeacon, Ownable {
    ...
    function startDraw() external override requireCanStartDraw {
        ...
        if (feeToken != address(0) && requestFee > 0) {
            ...
            uint beforeAllowance = IERC20(feeToken).allowance(address(this), address(rng));
            IERC20(feeToken).approve(address(rng), beforeAllowance + requestFee);
        }
