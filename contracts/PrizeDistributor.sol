...
/**
    * @title  luckanto PrizeDistributor
    * @author luckanto
...
contract PrizeDistributor is IPrizeDistributor, Ownable {
    ...
    /// claim _data arg pack function
    function packClaimArgs(uint64 [][] memory _pickIndicesForDraws) external view returns (bytes memory ret) {
        for(uint256 i; i < _pickIndicesForDraws.length; i++) {
            for(uint j = 1; j < _pickIndicesForDraws[i].length; j++) {
                require(_pickIndicesForDraws[i][j] > _pickIndicesForDraws[i][j - 1], "DrawCalc/picks-ascending");
            }
        }
        ret = abi.encode(_pickIndicesForDraws);
    }
