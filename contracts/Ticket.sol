...
contract Ticket is ControlledToken, ITicket {
    ...
    mapping(address => TwabLib.Account) public userTwabs;

    /// @notice Record of tickets total supply and ring buff parameters used for observation.
    TwabLib.Account public totalSupplyTwab;
    ...
    function controllerMint(address _user, uint256 _amount)
        external
        virtual
        override(ControlledToken, IControlledToken)
        onlyController
    {
        _mint(_user, _amount);
        _increaseUserTwab(_user, _amount);
        _increaseTotalSupplyTwab(_amount);
    }
    ...
    {
        _burn(_user, _amount);
        _decreaseUserTwab(_user, _amount);
        _decreaseTotalSupplyTwab(_amount);
    }
